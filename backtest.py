...
def send_discord_report(csv_file, total_tested):
    ...
    for _, row in sorted_df.iterrows():
        top_str += f"**{row['Ticker']}** | Trades: `{row['Trades']}` | ROI: `{row['ROI']:.1f}%`\n"

    embed = {
        "username": "Backtest Engine",
        "embeds": [{
            "title": "🧪 Max Frequency Backtest",
            "description": f"Tested **{total_tested}** stocks.",
            "color": 3447003,
            "fields": [
                {"name": "📈 Stats", "value": f"Profitable: **{profitable}**\nAvg Activity: **{avg_trades:.1f} trades**", "inline": True},
                {"name": "🏆 Most Active",
                    "value": top_str if top_str else "None", "inline": False}
            ],
            "footer": {"text": "Process Complete."}
        }]
    }
    try:
