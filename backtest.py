...
def load_tickers(filename):
    ...
    try:
        if filename.endswith('.xlsx'): df = pd.read_excel(filename)
    ...
    if 'Code' not in df_clean.columns: return []
    ...
    for i in range(200, len(df)):
        ...
        if active_trade:
            if curr['Low'] <= active_trade['sl']:
                pnl = (active_trade['sl'] - active_trade['entry']) * active_trade['shares']
                capital += pnl
                trades.append({'Result': 'LOSS', 'PnL': pnl})
                active_trade = None
            elif curr['High'] >= active_trade['tp']:
                pnl = (active_trade['tp'] - active_trade['entry']) * active_trade['shares']
                capital += pnl
        ...
        if past_window.empty: continue
        high = past_window['High'].max()
        low = past_window['Low'].min()
        diff = high - low
        if diff == 0: continue
        
        fib = {'0.0': high, '0.5': high - 0.5 * diff, '0.618': high - 0.618 * diff, '0.786': high - 0.786 * diff}
        ...
        trigger_stoch = (k_curr > d_curr) and (k_prev < d_prev) and (k_curr < 50)
        is_uptrend = (close > sma200) or (close > ema50)
        in_zone = (close >= fib['0.618'] * 0.85) and (close <= fib['0.5'] * 1.15)
        ...
        if trigger_stoch and is_uptrend and in_zone:
            ...
            if shares * entry_price > capital: shares = int(capital / entry_price)
            
            if shares > 0:
                active_trade = {'entry': entry_price, 'sl': sl, 'tp': tp, 'shares': shares}
...
def send_discord_report(csv_file, total_tested):
    ...
    embed = {
        ...
        "embeds": [{
            ...
            "color": 3447003, 
            "fields": [
                {"name": "📈 Statistik Global", "value": f"✅ Saham Profit: **{profitable}**\n🔄 Rata-rata Trade: **{avg_trades:.1f}x / saham**", "inline": False},
                {"name": "🏆 Top 10 Teraktif", "value": f"```prolog\n{top_str}```", "inline": False}
            ],
            "footer": {"text": "Mode Agresif: Stoch < 50, Zone Lebar 15%"}
        }]
    }
    try:
        with open(csv_file, "rb") as f: requests.post(DISCORD_WEBHOOK_URL, data={"payload_json": json.dumps(embed)}, files={"file": f})
    except: pass
            ...
            try:
                ...
                if df is not None:
                    ...
                    with open(csv_file, "a") as f:
                        f.write(f"{ticker},{res['Trades']},{wr:.1f},{res['Profit']:.0f},{roi:.1f},{res['DD']:.1f}\n")
            except: continue
        print("\n✅ Backtest Complete.")
