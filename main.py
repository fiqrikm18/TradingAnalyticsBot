...
def load_tickers_with_filter(filename, backtest_csv):
    ...
    try:
        if filename.endswith('.xlsx'): df_master = pd.read_excel(filename)
        else: df_master = pd.read_csv(filename)
    ...
    if 'Code' not in df_master.columns: return []
    all_tickers = [str(code) + ".JK" for code in df_master['Code'].tolist()]
    
    if os.path.exists(backtest_csv):
        try:
            df_bt = pd.read_csv(backtest_csv)
            good_stocks = df_bt[(df_bt['WinRate'] >= MIN_WIN_RATE) & (df_bt['Trades'] >= MIN_TRADES)]
            for _, row in good_stocks.iterrows():
                stock_stats[row['Ticker']] = {'wr': row['WinRate'], 'roi': row['ROI'], 'dd': row['MaxDD']}
            ...
            return final_list
        except: return all_tickers
    else: return all_tickers
...
def analyze_market_structure(close, sma200, ema50, adx):
    ...
    strength = "Weak"
    if close > sma200:
        if close > ema50: trend = "Strong Uptrend"
        else: trend = "Uptrend (Deep Pullback)"
    else:
        if close > ema50: trend = "Recovery Attempt"
...
def strategy_deep_dive(df, ticker):
    ...
    # 1. Trend
    trend_desc, trend_strength = analyze_market_structure(close, sma200, ema50, adx)
    if close > sma200 or close > ema50: score += 20
    else: warnings.append("Weak Trend")
    ...
    if stoch_cross and (score >= 40): 
        ...
        if score >= 70: signal = "💎 DIAMOND SETUP"
    ...
    if risk_per_share <= 0: risk_per_share = entry_price * 0.05
    ...
    rrr = (tp_price - entry_price) / risk_per_share if risk_per_share > 0 else 0
...
def generate_chart(df, ticker, data):
    ...
    ap_stoch = mpf.make_addplot(df['STOCHRSIk_14_14_3_3'], panel=1, color='cyan', ylabel='Stoch')
    ...
    try:
        mpf.plot(df, type='candle', style=s, title=f"{ticker} - Analysis", ylabel='IDR',
            ...
            hlines=dict(hlines=hlines, colors=colors, linewidths=[1]*4, linestyle='dashed'),
            volume=False, savefig=dict(fname=filename, dpi=100, pad_inches=0.25))
    except: return None
    return filename

def send_discord_alert(data, chart_path):
    if "WAIT" in data['signal']: return
    
    # Warna: Biru Tua (70+), Biru Biasa (<70)
    ...
    # Format Data Backtest
    hist_stats = stock_stats.get(data['ticker'], None)
    if hist_stats:
        stats_display = f"Win Rate: {hist_stats['wr']:.0f}% | Max DD: {hist_stats['dd']:.1f}%"
    else:
        stats_display = "Data Backtest Tidak Tersedia"

    # --- BLOCK TEXT UNTUK FORMAT RAPI ---
    execution_block = f"""```yaml
ENTRY : {data['entry']:,.0f} (Market)
TP    : {data['tp']:,.0f}
SL    : {data['sl']:,.0f} (-{risk['sl_pct']:.1f}%)
```"""

    money_block = f"""```prolog
Lots  : {data['lots']} Lot
Modal : Rp {data['capital_req']:,.0f}
Risk  : Rp {data['pnl']['loss']:,.0f} (2%)
```"""
    ...
    embed = {
        ...
        "embeds": [{
            ...
            "fields": [
                {
                    "name": "🚥 Rencana Eksekusi",
                    "value": execution_block,
                    "inline": False
                },
                {
                    "name": "💰 Manajemen Uang & Risiko",
                    "value": money_block,
                    "inline": False
                },
                {
                    "name": "📊 Konteks Pasar",
                    "value": f"**Vol:** {thesis['volume']} | **RSI:** {thesis['rsi']}\n*{stats_display}*",
                    "inline": False
                },
                {
                    "name": "🛡️ Peringatan Risiko",
                    "value": f"Resiko per Lot: `Rp -{risk['risk_lot']:,.0f}`\nMove BEP di: `{risk['be_trigger']:,.0f}`",
                    "inline": False
                }
            ],
            "footer": {"text": f"Dianalisa pada {datetime.now().strftime('%H:%M')} WIB"}
        }]
    }
    if chart_path and os.path.exists(chart_path):
        with open(chart_path, "rb") as f: requests.post(DISCORD_WEBHOOK_URL, data={"payload_json": json.dumps(embed)}, files={"file": f})
    else: requests.post(DISCORD_WEBHOOK_URL, data={"payload_json": json.dumps(embed)})
